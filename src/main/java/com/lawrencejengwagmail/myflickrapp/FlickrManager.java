...
import com.lawrencejengwagmail.myflickrapp.MainActivity.UIHandler;
...
public class FlickrManager {
    ...
    //Lawrence Flickr API_KEY
    private static final String APIKEY_SEARCH_STRING = "5de5710f69db80c69a975ba70c73328a";
    ...
    public static void getImageURLS(ImageContainer imgCon) {
    ...
    public static Bitmap getImage(ImageContainer imgCon) {
        Bitmap bm = null;
        try {
    ...
    public static void getThumbnails(ArrayList<ImageContainer> imgCon, UIHandler uih) {
        for (int i = 0; i < imgCon.size(); i++)
            new GetThumbnailsThread(uih, imgCon.get(i)).start();
    }

    public static Bitmap getThumbnail(ImageContainer imgCon) {
        Bitmap bm = null;
        try {
    ...
    public static class GetThumbnailsThread extends Thread {
        ...
        private static final ImageContainer ImageContainer = Image;
        UIHandler uih;
        ...
        public GetThumbnailsThread(UIHandler uih, ImageContainer imgCon) {
            this.uih = uih;
        ...
        public void run() {
            ...
            ImageContainer.thumb= getThumbnail(ImageContainer);
            if (ImageContainer.thumb != null) {
    ...
    public static ArrayList<ImageContainer> searchImagesByTag(UIHandler uih, Context ctx, String tag) {
        uihandler = uih;
        String url = createURL(FLICKR_PHOTOS_SEARCH_ID, tag);
        ArrayList<ImageContainer> tmp = new ArrayList<ImageContainer>();
        ...
        try {
            ...
            try {
                ...
                for (int i = 0; i < imageJSONArray.length(); i++) {
                    ...
                    ImageContainer imgCon = new ImageContainer(item.getString("id"), item.getString("owner"), item.getString("secret"), item.getString("server"),
                            item.getString("farm"));
                    imgCon.position = i;
                    tmp.add(imgCon);
                }
